...
import SkeletonLoader from '../components/SkeletonLoader';
  ...
  const [metrics, setMetrics] = useState({ today: 0, overdue: 0, followups: 0, proposalsCreated: 0, proposalsSent: 0, proposalsSigned: 0, proposalConversion: 0, avgValue: 0, financingMix: {} });
  ...
  async function fetchMetrics() {
    ...
    try {
      ...
      if (proposals) {
        ...
        avgValue = active.length ? Math.round(active.reduce((sum: number, p: any) => sum + (Number(p.monthly_bill) || 0), 0) / active.length) : 0;
      ...
      setMetrics({ today: todayCount, overdue: overdueCount, followups, proposalsCreated, proposalsSent, proposalsSigned, proposalConversion, avgValue, financingMix });
    } catch (e: any) {
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div style={{background:'#fff3f3',color:'#b00020',padding:'1rem 2rem',borderRadius:8,margin:'2rem 0',textAlign:'center'}}>
          <div style={{fontWeight:600}}>Unable to load dashboard metrics.</div>
          <div style={{margin:'1em 0'}}>{error}</div>
          <button onClick={fetchMetrics} style={{background:'#2b3990',color:'#fff',border:'none',padding:'0.5em 1.5em',borderRadius:4}}>Try Again</button>
        </div>
      ) : (
        <div style={{display:'flex',gap:24,flexWrap:'wrap'}}>
          ...
          <div style={{background:'#fff',color:'#2b3990',padding:'1rem 2rem',borderRadius:8,border:'1px solid #2b3990',minWidth:180}}>
            ...
            <div>
              {Object.keys(metrics.financingMix).length === 0 ? (
                ...
                Object.entries(metrics.financingMix).map(([k,v]) => (
                  <div key={k}>{k}: {v}</div>
                ))
              )}
            </div>
